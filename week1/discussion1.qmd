...
fracking_clean <- fracking |> 
  ...
  # clean column names ----
  janitor::clean_names() |> # update column headers to a snake case "_"
  
  # clean up dates ----
  mutate(job_start_date = str_remove(job_start_date, " AM")) |> # remove 'AM' from the job_start_date column
  mutate(datetime_start = mdy_hms(job_start_date)) |> # convert job_start_date from string to date and save to new col datetime_start
  mutate(year = year(datetime_start)) |> # retrieve a year value from datetime_start into a new col 'year'

  # select relevant cols ----
  select(datetime_start, year, state_name, county_name, well_name, total_base_water_volume) |> 
  
  # filter out non-state names values from col state_name ----
  filter(!state_name %in% c("Beaver", "Beckham", "Harper", "Hemphill", "Midland", "Red River", "Roosevelt", "Rusk", "State", "WARD")) |> 
  
  # rename state_name col header to something shorter for typing out when using case_when (not necessary) ----
  rename(sn = state_name) |> 
  
  # update individual words within all columns to title case ----
  mutate(sn = str_to_title(sn)) |> 
  
  # fix misspelled state names in sn col ----
  mutate(sn = case_when(
    ...
    sn == "Wyominng" ~ "Wyoming", 
    TRUE ~ sn # apply default action: copy over rest of state names from as-is
  )) |> 
  
  # remove rows that have uncertainty (values that have a '?' mark) ----
  filter(!str_detect(string = sn, pattern = "\\?")) |> # `?` is a special chr; escape with `\\` prefix
  
  # update state values within col "sn" to all uppercase (so that we can covert abbreviation to state names) ----
  mutate(sn = str_to_upper(sn)) |> 
  
  # mutate abbreviations to full state names ----
  # convert abbreviated state names to full state names in col "sn"
  mutate(sn = ifelse(test = str_length(sn) == 2, # if string in 'sn' col is 2 chrs long
                     ...
                     no = sn)) |> # apply a default action: if string in 'sn' col is not 2 chrs long, keep state name as-is
...
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# OP: q. to what extent water distribution varies between the states?
